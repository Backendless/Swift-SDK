...
class SharedObjectTests: XCTestCase {
    ...
    func test_12_addCommandListener() {
        let expectation: XCTestExpectation = self.expectation(description: "PASSED: rso.addCommandListener")
        let _ = self.sharedObject.addCommandListener(responseHandler: { commandObject in
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2, execute: {
            ...
            self.sharedObject.sendCommand(commandName: "TestCommand", data: message, responseHandler: {
            }, errorHandler: { fault in
                XCTAssertNotNil(fault)
                XCTFail("\(fault.code): \(fault.message!)")
            })
    ...
    func test_13_removeCommandListeners() {
        let expectation: XCTestExpectation = self.expectation(description: "PASSED: rso.removeCommandListeners")
        let _ = self.sharedObject.addCommandListener(responseHandler: { commandObject in
            XCTFail("This subscription must be removed")
        }, errorHandler: { fault in
            XCTAssertNotNil(fault)
        ...
        sharedObject.removeCommandListeners()
        DispatchQueue.main.asyncAfter(deadline: .now() + 2, execute: {
            let message = "Test Message"
            self.sharedObject.sendCommand(commandName: "TestCommand", data: message, responseHandler: {
            }, errorHandler: { fault in
    ...
    func test_14_stopCommandSubscription() {
        let expectation: XCTestExpectation = self.expectation(description: "PASSED: rso.stopCommandSubscription")
        let subscriptionToStop = self.sharedObject.addCommandListener(responseHandler: { commandObject in
            XCTFail("This subscription must be removed")
        }, errorHandler: { fault in
            XCTAssertNotNil(fault)
            XCTFail("\(fault.code): \(fault.message!)")
    ...
    // TODO after BKNDLSS-18326 fixed
    
    /*func test_15_addUserStatusListener() {
    }
    
    func test_16_removeUserStatusListeners() {
    }
    
    func test_17_stopUserStatusSubscription() {
    }
    
    func test_18_addInvokeObjectListener() {
    }
    
    func test_19_removeInvokeObjectListeners() {
    }
    
    func test_20_stopInvokeObjectSubscription() {
    }*/
}
