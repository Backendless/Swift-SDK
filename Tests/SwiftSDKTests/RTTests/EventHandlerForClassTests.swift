...
class EventHandlerForClassTests: XCTestCase {
    ...
    func test_24_addSetRelationListener() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.addSetRelationListener")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3, execute: {
            let objectToSave = self.createTestClassObject()
            self.dataStore.save(entity: objectToSave, responseHandler: { savedObject in
                let childObjectToSave = self.createChildTestClassObject()
                self.childDataStore.save(entity: childObjectToSave, responseHandler: { savedChild in
                    if let parentId = (savedObject as? TestClass)?.objectId,
                        let childId = (savedChild as? ChildTestClass)?.objectId {
    ...
    func test_25_removeSetRelationListeners() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.removeSetRelationListener")
    ...
    func test_26_addAddRelationListener() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.addAddRelationListener")
    ...
    func test_27_removeAddRelationListeners() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.removeAddRelationListener")
    ...
    func test_28_addDeleteRelationListener() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.addDeleteRelationListener")
    ...
    func test_29_removeDeleteRelationListeners() {
        let expectation = self.expectation(description: "PASSED: eventHandlerForClass.removeDeleteRelationListener")
